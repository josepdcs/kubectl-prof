...
import (
	"github.com/josepdcs/kubectl-prof/pkg/agent/test"
		...
		},
		{
			...
			mockFunc: func() {
				crioConfigFile = func(string) string {
					return filepath.FromSlash(test.CrioTestDataDir() + "/other.json")
				}
			},
...
func TestRuntimeSpec(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			configFile:      filepath.FromSlash(test.CrioTestDataDir() + "/wrong.json"),
...
func TestPID(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mockFunc: func() {
				crioStateFile = func(string) string {
					return filepath.FromSlash(test.CrioTestDataDir() + "/state.json")
				}
			},
			expected: "3854",
		},
...
func TestRuntimeState(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "expected runtime spec",
			configFile: filepath.FromSlash(test.CrioTestDataDir() + "/state.json"),
			expected:   "3854",
		},
		{
			name:            "unexpected runtime spec",
