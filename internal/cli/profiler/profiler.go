...
type Profiler struct {
	Getter  kubernetes2.Getter
	Creator kubernetes2.Creator
	Deleter kubernetes2.Deleter
}

//NewProfiler returns new Profiler
func NewProfiler(get kubernetes2.Getter, cre kubernetes2.Creator, del kubernetes2.Deleter) *Profiler {
	return &Profiler{
		Getter:  get,
		Creator: cre,
		Deleter: del,
...
func (p *Profiler) Profile(cfg *config2.ProfilerConfig) {
	...
	containerId, err := kubernetes2.ToContainerId(containerName, pod)
	if err != nil {
		printer.PrintError()
		log.Fatalf(err.Error())
	}
...
func validatePod(pod *v1.Pod, cfg *config2.TargetConfig) (string, error) {
	if pod == nil {
