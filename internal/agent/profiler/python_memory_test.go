	...
	tests := []struct {
	...
	}{
		{
			...
			given: func() (fields, args) {
				...
				file.Write(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"flamegraph-1000-1.html"), b.String())
...
func Test_memrayManager_handleReport(t *testing.T) {
	type fields struct {
		MemrayProfiler *MemrayProfiler
	}
	type args struct {
	...
	tests := []struct {
		...
		{
			name: "should handle flamegraph report",
			given: func() (fields, args) {
				var b bytes.Buffer
				b.Write([]byte("test"))
				_ = os.WriteFile(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"raw.bin"), b.Bytes(), 0644)
				...
				return fields{
						...
						resultFileName: filepath.Join(common.TmpDir(), config.ProfilingPrefix+"flamegraph.html"),
					}
			},
			when: func(fields fields, args args) error {
			...
			after: func() {
				_ = file.Remove(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"raw.bin"))
			},
		},
		{
			name: "should handle summary report",
			given: func() (fields, args) {
				commander := executil.NewMockCommander()
				commander.On("Command").Return(exec.Command("echo", "summary output"))
				publisher := publish.NewFakePublisher()
				...
				return fields{
						...
						rawFileName:    filepath.Join(common.TmpDir(), config.ProfilingPrefix+"raw.bin"),
						resultFileName: filepath.Join(common.TmpDir(), config.ProfilingPrefix+"summary.txt"),
					}
			},
			when: func(fields fields, args args) error {
				return fields.MemrayProfiler.handleReport(args.job, args.rawFileName, args.resultFileName)
			...
			then: func(t *testing.T, err error, fields fields) {
				assert.Nil(t, err)
				assert.True(t, file.Exists(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"summary.txt")))
			},
			after: func() {
				_ = file.Remove(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"summary.txt"))
			},
		},
		{
			name: "should handle tree report",
			given: func() (fields, args) {
				commander := executil.NewMockCommander()
				commander.On("Command").Return(exec.Command("echo", "tree output"))
				publisher := publish.NewFakePublisher()
				...
				return fields{
						...
						rawFileName:    filepath.Join(common.TmpDir(), config.ProfilingPrefix+"raw.bin"),
						resultFileName: filepath.Join(common.TmpDir(), config.ProfilingPrefix+"tree.txt"),
					}
			},
			when: func(fields fields, args args) error {
				return fields.MemrayProfiler.handleReport(args.job, args.rawFileName, args.resultFileName)
			...
			then: func(t *testing.T, err error, fields fields) {
				assert.Nil(t, err)
				assert.True(t, file.Exists(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"tree.txt")))
			},
			after: func() {
				_ = file.Remove(filepath.Join(common.TmpDir(), config.ProfilingPrefix+"tree.txt"))
			},
		},
		{
			name: "should fail handle report when command fails",
			given: func() (fields, args) {
				commander := executil.NewMockCommander()
				commander.On("Command").Return(&exec.Cmd{})
				publisher := publish.NewFakePublisher()
