...
const (
	...
	phpDelayBetweenJobs      = 2 * time.Second
)
...
func NewPhpProfiler(commander executil.Commander, publisher publish.Publisher) *PhpProfiler {
	return &PhpProfiler{
		delay: phpDelayBetweenJobs,
...
func (p *PhpProfiler) SetUp(job *job.ProfilingJob) error {
...
func (p *PhpProfiler) Invoke(job *job.ProfilingJob) (error, time.Duration) {
	start := time.Now()
...
func (m *phpManager) invoke(job *job.ProfilingJob, pid string) (error, time.Duration) {
	start := time.Now()
	err := m.runPhpRecord(job, pid)
	if err != nil {
...
func (m *phpManager) foldPhpOutput(job *job.ProfilingJob, pid string) (error, string) {
...
func (m *phpManager) handleFlamegraph(job *job.ProfilingJob, flameGrapher flamegraph.FrameGrapher, rawFileName string,
...
func (p *PhpProfiler) CleanUp(job *job.ProfilingJob) error {
	file.RemoveAll(common.TmpDir(), "php")
