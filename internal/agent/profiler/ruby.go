...
func (r *RubyProfiler) Invoke(job *config.ProfilingJob) error {
	...
	cmd := utils2.Command(rbspyLocation, "record", "--pid", pid, "--file", filName, "--duration", duration, "--format", "flamegraph")
	...
	return utils2.Publish(job.Compressor, filName, job.OutputType)
}
