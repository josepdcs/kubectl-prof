...
func (m *mockPerfUtil) runPerfRecord(job *config.ProfilingJob) error {
	args := m.Called(job)
	return args.Error(0)
}

func (m *mockPerfUtil) runPerfScript(job *config.ProfilingJob) error {
	args := m.Called(job)
	return args.Error(0)
}

func (m *mockPerfUtil) foldPerfOutput(job *config.ProfilingJob) error {
	args := m.Called(job)
	return args.Error(0)
}

func (m *mockPerfUtil) generateFlameGraph(job *config.ProfilingJob) error {
	args := m.Called(job)
	return args.Error(0)
}

func (m *mockPerfUtil) publishResult(c api.Compressor, fileName string, outputType api.EventType) error {
...
func TestPerfProfiler_SetUp(t *testing.T) {
	...
	assert.Equal(t, nil, p.SetUp(job))
}
