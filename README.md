...
[JFR](https://docs.oracle.com/javacomponents/jmc-5-4/jfr-runtime-guide/about.htm#) format can be generated by adding `-o/--output jfr` option. 
Even profiling tool can be chosen by passing option `--tool`. See below.
...
```shell
kubectl prof mypod -f flight.jfr -t 5m -l java -o jfr 
```
...
```shell
kubectl prof mypod -f flight.jfr -t 5m -l java -o jfr --tool jcmd
```
